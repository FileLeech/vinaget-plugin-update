		...
		for ($k=0; $k < $maxacc; $k++){
			...
			if(empty($cookie)==false || ($user && $pass)){
				for ($j=0; $j < 2; $j++){
					...
					if (stristr($page,'title="Tập tin có mật khẩu bảo vệ"></i>')) {  // Link Password
						if($password) {
							if (preg_match('/<input type="hidden" value="(.*?)" name="fs_csrf" \/>/', $page, $value)) $fs_csrf = $value[1];
							$link = ""; $k = 0;
							while ($k < 5) {
								$content = json_decode(new_curl("https://www.fshare.vn/download/get", $url, $cookie, "DownloadForm[pwd]={$password}&fs_csrf={$fs_csrf}&ajax=download-form"), true);
								if (isset($content['DownloadForm_pwd'])) die($this->lang['reportpass']);
